...
> **Highly experimental.** The design _should_ work on macOS, Linux, and Windows (via WSL) hosts, and with any IDE that integrates with Claude Code — but it has only been **tested on a macOS / Apple Silicon (arm64) host with Neovim**. The `node_modules` overlay is a macOS-only workaround (skipped on Linux, where the bind-mounted `node_modules` is already the right OS/arch). Intel Macs are not supported. Because the whole feature is opt-in, none of this affects anyone who doesn't run `sbx`. Expect rough edges off the tested path; reports/fixes welcome.
...
> **node_modules:** dependencies are installed into a **container-local** `node_modules`, kept separate from the host's. On macOS the host's `node_modules` holds macOS-native binaries the Linux sandbox can't run, so on each mount the script overlays `node_modules` with a container-local dir (`sudo mount --bind` — your host `node_modules` is never touched) and runs `pnpm install` into it on first mount (a few minutes; reused while the lockfile is unchanged). On a Linux host the bind-mounted `node_modules` is already the right OS/arch, so this is skipped. If you change dependencies while the sandbox is up, refresh without re-mounting via `./scripts/sbx.sh refresh-deps`.
