...
describe('useIsDelayedFetchingMore', () => {
    beforeEach(() => {
        vi.useFakeTimers()
    })
    ...
    it('should initialize with loading state false', () => {
        const { result } = renderHook(() => useIsDelayedFetchingMore())

        expect(result.current.isDelayedFetchingMore).toBe(false)
    })
    ...
    it('should set loading to true after SHOW_DELAY (250ms)', async () => {
        ...
        act(() => {
            result.current.startDelayedFetchingMore()
        })
        ...
        expect(result.current.isDelayedFetchingMore).toBe(true)
    })

    it('should clear loading state immediately if completeDelayedFetchingMore is called before SHOW_DELAY', async () => {
        const { result } = renderHook(() => useIsDelayedFetchingMore())
        ...
        await act(() => result.current.completeDelayedFetchingMore())
        expect(result.current.isDelayedFetchingMore).toBe(false)
    ...
    it('should wait for remaining time if loading lasted less than MIN_LOAD_DURATION', async () => {
        ...
        // Call completeDelayedFetchingMore - loading has lasted 200ms,
        // so it needs to wait 200ms more (400ms MIN_LOAD_DURATION - 200ms elapsed)
        result.current.completeDelayedFetchingMore()
        expect(result.current.isDelayedFetchingMore).toBe(true)
    ...
    it('should clear timeouts on unmount', async () => {
        ...
        const { result, unmount } = renderHook(() => useIsDelayedFetchingMore())
