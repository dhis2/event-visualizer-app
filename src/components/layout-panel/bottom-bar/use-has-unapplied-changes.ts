...
export const useHasUnappliedChanges = (): boolean => {
    ...
    return useMemo(() => {
        ...
        try {
             ...
             * layout being the reachable case — has nothing to apply, which is
             * what the disabled output type buttons already tell the user.
             * This runs on every canvas render, so throwing here would take
             * the whole app down instead. */
            logger.error(
